...
<div class="container mt-5">
    <div class="card shadow-sm">
        <div class="card-header bg-warning text-dark">
            <h1 class="card-title h4 mb-0">تعديل فاتورة مبيعات رقم #{{ $salesInvoice->id }}</h1>
        </div>
        <div class="card-body">
            <form action="{{ route('sales-invoices.update', $salesInvoice->id) }}" method="POST">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="branch_id" id="branch_id" class="form-select" required>
                            @foreach ($branches as $branch)
                                <option value="{{ $branch->id }}" @selected(old('branch_id', $salesInvoice->branch_id) == $branch->id)>{{ $branch->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                        <label for="date" class="form-label fw-bold">التاريخ</label>
                        <input type="date" name="date" id="date" class="form-control" value="{{ old('date', \Carbon\Carbon::parse($salesInvoice->date)->format('Y-m-d')) }}" required>
                    </div>
                    <div class="col-md-3">
                        <label for="payment_method" class="form-label fw-bold">طريقة الدفع</label>
                        <select name="payment_method" id="payment_method" class="form-select">
                            <option @selected(old('payment_method', $salesInvoice->payment_method) == 'نقدا')>نقدا</option>
                            <option @selected(old('payment_method', $salesInvoice->payment_method) == 'بطاقة')>بطاقة</option>
                            <option @selected(old('payment_method', $salesInvoice->payment_method) == 'أخرى')>أخرى</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <label for="status" class="form-label fw-bold">الحالة</label>
                        <select name="status" id="status" class="form-select">
                            <option @selected(old('status', $salesInvoice->status) == 'مدفوع')>مدفوع</option>
                            <option @selected(old('status', $salesInvoice->status) == 'معلق')>معلق</option>
                            <option @selected(old('status', $salesInvoice->status) == 'ملغى')>ملغى</option>
                        </select>
                    </div>
                    <div class="col-md-12 mt-3">
                        <label for="note" class="form-label fw-bold">ملاحظات (اختياري)</label>
                        <input type="text" name="note" id="note" class="form-control" value="{{ old('note', $salesInvoice->note) }}">
                    </div>
                </div>
                    ...
                    @foreach($salesInvoice->items as $index => $item)
                        <div class="row item-row gx-2 gy-2 align-items-center mb-3 p-3 border rounded bg-light">
                            <div class="col-lg-5">
                                ...
                                <input list="batches-list" name="items[{{ $index }}][batch_id]" class="form-control batch-input" value="{{ $item->batch_id }}" required>
                            </div>
                            <div class="col-lg-2 col-md-4">
                                <label class="form-label">السعر</label>
                                <div class="input-group">
                                     <span class="input-group-text">جنيه</span>
                                     <span class="form-control bg-white price-span">{{ number_format($item->price, 2) }}</span>
                                </div>
                            </div>
                            <div class="col-lg-2 col-md-4">
                                <label class="form-label">الكمية المطلوبة</label>
                                <input type="number" name="items[{{ $index }}][quantity]" class="form-control quantity-input" value="{{ $item->qty }}" min="0.1" step="0.1" required>
                            </div>
                            <div class="col-lg-2 col-md-4">
                                 <label class="form-label">الإجمالي الجزئي</label>
                                 <div class="input-group">
                                     <span class="input-group-text">جنيه</span>
                                     <span class="form-control bg-white subtotal-span">{{ number_format($item->total, 2) }}</span>
                                </div>
                            </div>
                ...
                <div class="mt-5 d-flex justify-content-end align-items-center">
                    <h4 class="me-4">الإجمالي: <span id="total-amount" class="fw-bold">{{ number_format($salesInvoice->total, 2) }}</span> جنيه</h4>
...
<datalist id="batches-list">
    @foreach ($availableBatches as $batch)
        <option value="{{ $batch->id }}" data-price="{{ $batch->selling_price }}" data-quantity="{{ $batch->quantity + ($salesInvoice->items->firstWhere('batch_id', $batch->id)->qty ?? 0) }}">
            {{ $batch->medicine->name }} (متوفر: {{ $batch->quantity + ($salesInvoice->items->firstWhere('batch_id', $batch->id)->qty ?? 0) }}) - [{{ $batch->branch->name }}] - (انتهاء: {{ \Carbon\Carbon::parse($batch->expiry_date)->format('Y-m') }})
        </option>
    @endforeach
</datalist>
